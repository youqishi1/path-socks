...
执行安装命令可直接选择 **6 彻底卸载整个项目**，不必先重装或升级代理。新版管理菜单也支持 `sbb` → 6，或直接 `sbb purge`。需要联网下载卸载器。

先列出删除清单，再依次输入 `yes`、`PURGE-SBB` 才执行。永久删除受管的两套代理、独立Nginx入口、续期任务、配置/UUID/Token/证书、项目日志、历史备份及共享sbb命令；删除未被其他用户使用的专用账号。**不创建新备份，不可由工具恢复。** 取消任一确认不会卸载。

不卸载共用Nginx、Certbot、Python等软件；不清空系统journal；旧版未标注归属的防火墙规则/云安全组、全局共享证书、手动下载的源码和客户端配置需另外确认归属。发现旧版`/etc/nginx/sites-*/path-socks`会在删除前停止，不能冒险改动未知共享实例。遇到挂载目录、异常服务来源或仍运行的项目进程也会停止并报告。不是删除整台VPS上的所有相关软件。
